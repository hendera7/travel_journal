...
In the table's row component file, truncate the resulting date's number of characters to 10 by adding this to the table row's date column: {exercise.date.slice(0,10)}. This will keep the time characters from showing when using the <input type="date" /> date picker.
